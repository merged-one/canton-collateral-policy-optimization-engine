...
- first Quickstart-backed end-to-end margin-call demo command with a machine-readable execution report, Markdown summary, timeline artifacts, and subordinate workflow plus adapter artifacts
- first end-to-end return demo command with a machine-readable return report, Markdown summary, and timeline artifacts
- first Quickstart-backed end-to-end return demo command with a machine-readable return report, Markdown summary, timeline artifacts, and subordinate workflow plus adapter artifacts
...
- containerized Quickstart runtime bridge plus real Quickstart start, deploy, seed, and status commands for the Control Plane DAR and seeded scenario
- first Quickstart-backed reference token adapter path with a machine-readable adapter execution report and provider-visible adapter status artifacts
...
- production-grade live integrations with custodians, CCPs, central-bank systems, or external pricing stacks
- Quickstart-backed adapter coverage beyond the current seeded scenario plus narrow reference token posting path in the pinned LocalNet
- generic external integration buses or production custodian abstractions
- UI development
- performance tuning
...
`make localnet-bootstrap` now stages a pinned upstream CN Quickstart checkout and writes a repo-owned `.env.local` overlay without forking upstream files. `make localnet-smoke` reuses upstream Docker preflight checks and validates the composed Quickstart LocalNet configuration. `make localnet-build-dar` builds the Control Plane DAR against the pinned Quickstart runtime line through a containerized Daml `3.4.10` plus Java `21` bridge. `make localnet-deploy-dar` rebuilds that DAR and installs it into the running Quickstart app-provider and app-user participants through the upstream onboarding container. `make localnet-start-control-plane` starts the repo-owned isolated Quickstart overlay, deploys the Control Plane DAR, and writes deployment receipts. `make localnet-seed-demo` allocates the required parties and users, seeds one confidential margin-style collateral scenario on Quickstart, and writes contract receipts. `make localnet-status-control-plane` queries the running LocalNet from the provider-visible view and writes machine-readable plus Markdown status snapshots. `make localnet-run-token-adapter` consumes the seeded `SettlementInstruction`, performs the reference token movement on Quickstart, confirms workflow settlement, and emits a schema-valid adapter execution report. `make localnet-adapter-status` refreshes the provider-visible adapter receipt and holding surface. `make demo-run` exercises a real Daml workflow smoke script over the initial obligation, posting, substitution, and return skeletons. `make demo-margin-call` evaluates positive and negative margin-call scenarios, passes the positive recommendation into a Daml Script workflow path, and emits a schema-valid `ExecutionReport` plus Markdown summary and timeline artifacts on the IDE ledger. `make demo-margin-call-quickstart` runs the Quickstart-backed margin-call chain end to end across policy evaluation, optimization, Quickstart workflow preparation, reference token adapter execution, and final execution reporting. `make demo-return` evaluates the currently encumbered set, derives the returned lots from a deterministic retained-set recommendation, enforces approvals plus replay-safe release control on the Daml boundary, and emits a schema-valid `ReturnReport` plus Markdown summary and timeline artifacts. `make demo-return-quickstart` now runs the Quickstart-backed return chain end to end across retained-set selection, Quickstart workflow execution, reference token adapter release, provider-visible status refresh, approval-gated release proof, and replay-safe duplicate-request blocking. `make demo-substitution` starts from already encumbered collateral, evaluates substitution-specific positive and negative scenarios, enforces approvals plus atomicity on the Daml boundary, and emits a schema-valid `SubstitutionReport` plus Markdown summary and timeline artifacts. `make demo-substitution-quickstart` now runs the Quickstart-backed substitution chain end to end across policy evaluation, replacement selection, Quickstart substitution workflow execution, reference token adapter release plus replacement movement, provider-visible status refresh, and final substitution reporting. `make test-conformance` now refreshes Quickstart deployment evidence, validates one concrete reference token adapter proof path, and validates the three Quickstart-backed confidential workflow demo artifact chains before emitting aggregate invariant pass or fail output. `make demo-all` now packages those runtime-backed artifacts, the conformance output, the readiness assessment, and the integration-guide references into one final machine-readable demo pack. `make daml-test` runs the script-level lifecycle checks individually. `make policy-eval` validates a real policy input, evaluates normalized inventory, and emits a schema-valid `PolicyEvaluationReport`. `make optimize` validates a real policy input, optimizes against normalized inventory plus obligation inputs, and emits a schema-valid `OptimizationReport`. `make verify-portable` runs the full repository verification loop without the Docker-dependent Quickstart smoke gate. `make verify` remains the full superset and adds `make localnet-smoke` on top of the portable checks.
...
- [examples/demo-scenarios/margin-call/quickstart-demo-config.json](./examples/demo-scenarios/margin-call/quickstart-demo-config.json)
...
- [reports/generated/margin-call-quickstart-execution-report.json](./reports/generated/margin-call-quickstart-execution-report.json)
- [reports/generated/margin-call-quickstart-summary.md](./reports/generated/margin-call-quickstart-summary.md)
- [reports/generated/margin-call-quickstart-timeline.md](./reports/generated/margin-call-quickstart-timeline.md)
...
- [examples/demo-scenarios/substitution/quickstart-demo-config.json](./examples/demo-scenarios/substitution/quickstart-demo-config.json)
...
- [reports/generated/substitution-quickstart-report.json](./reports/generated/substitution-quickstart-report.json)
- [reports/generated/substitution-quickstart-summary.md](./reports/generated/substitution-quickstart-summary.md)
- [reports/generated/substitution-quickstart-timeline.md](./reports/generated/substitution-quickstart-timeline.md)
...
- [reports/generated/return-quickstart-report.json](./reports/generated/return-quickstart-report.json)
- [reports/generated/return-quickstart-summary.md](./reports/generated/return-quickstart-summary.md)
- [reports/generated/return-quickstart-timeline.md](./reports/generated/return-quickstart-timeline.md)
