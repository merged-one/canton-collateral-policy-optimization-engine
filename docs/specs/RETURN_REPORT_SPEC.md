...
- Primary command: `make demo-return`
...
- the committed or blocked Daml return workflow result for each scenario
...
- workflow metadata such as the return request ID, correlation ID, custody account, settlement accounts, and simulated control-failure flags
...
8. The top-level `returnReportId` is a stable hash of the manifest content plus the scenario results used to build the report.
...
The current positive scenario emits:
...
- one blocked unauthorized release path
- one replay-protection path that closes the original return and then blocks the duplicate instruction
- one obligation-state mismatch path that fails before approval or settlement can progress
...
Those remain future work for CPL evolution, workflow disclosure profiles, Quickstart deployment, and the broader conformance suite.
