...
| RUNTIME-001 | Runtime and demo separation | LocalNet overlays, demo bootstrap data, and runtime services must not alter policy semantics, bypass approvals, or fabricate successful reports. | deployment model, ADR 0015, ADR 0016, Quickstart README, `make localnet-smoke`, `make localnet-build-dar`, `make localnet-deploy-dar`, `make demo-margin-call`, `make demo-return`, `make demo-substitution`, `make demo-all`, prompt evidence |
...
| TOOL-001 | Toolchain reproducibility | Build, validation, and smoke-run commands must resolve to pinned Daml, Java, validation-tool, and Quickstart versions that are documented, checksum-verified or commit-pinned where downloaded, reproducible from a clean checkout, and explicitly tiered so portable verification does not silently depend on optional Docker-backed runtime checks. | dependency policy, bootstrap script, Quickstart bootstrap script, Quickstart DAR bridge script, `scripts/verify-portable.sh`, `make verify-portable`, `make verify`, `make daml-build`, `make daml-test`, `make demo-run`, `make localnet-smoke`, `make localnet-build-dar`, `make localnet-deploy-dar`, verification commands |
