...
| WF-001 | Workflow authority | Obligation, encumbrance, approval, and settlement state may change only through committed workflow transitions on Canton; off-ledger services may propose or report changes but may not authoritatively apply them. | `daml/CantonCollateral/*.daml`, ADR 0007, Daml test plan, execution evidence, `make localnet-seed-demo`, Quickstart seed receipt |
| RUNTIME-001 | Runtime and demo separation | LocalNet overlays, demo bootstrap data, and runtime services must not alter policy semantics, bypass approvals, or fabricate successful reports. | deployment model, ADR 0015, ADR 0016, ADR 0017, Quickstart README, LocalNet control-plane runbook, `make localnet-smoke`, `make localnet-build-dar`, `make localnet-deploy-dar`, `make localnet-start-control-plane`, `make localnet-seed-demo`, `make localnet-status-control-plane`, `make demo-margin-call`, `make demo-return`, `make demo-substitution`, `make demo-all`, prompt evidence |
