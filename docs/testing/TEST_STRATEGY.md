...
- `make localnet-start-control-plane`, `make localnet-seed-demo`, `make localnet-status-control-plane`, `make localnet-run-token-adapter`, and `make localnet-adapter-status` are operator-facing Quickstart checks that currently sit outside the default verify targets because they require a running Docker stack, Quickstart auth wiring, and a seeded LocalNet.
