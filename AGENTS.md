...
- A first aggregate conformance suite now exists under `make test-conformance`, generating explicit invariant pass/fail output plus supporting determinism and haircut evidence across the margin-call, substitution, and return demos.
- A first final demo pack now exists under `make demo-all`, generating a machine-readable package index plus Markdown summary for the full confidential workflow demo surface.
