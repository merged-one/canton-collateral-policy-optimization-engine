...
- A pinned CN Quickstart bootstrap, containerized Quickstart-DAR bridge build, isolated overlay start path, package-deployment command, seeded confidential collateral scenario, and Quickstart status-evidence surface now exist under `infra/quickstart/` plus `scripts/`.
...
- Reproducible control commands now include `make bootstrap`, `make localnet-bootstrap`, `make localnet-smoke`, `make localnet-build-dar`, `make localnet-deploy-dar`, `make localnet-start-control-plane`, `make localnet-seed-demo`, `make localnet-status-control-plane`, `make validate-cpl`, `make policy-eval`, `make optimize`, `make test-policy-engine`, `make test-optimizer`, `make test-conformance`, `make daml-build`, `make daml-test`, `make demo-run`, `make demo-margin-call`, `make demo-return`, `make demo-substitution`, `make demo-all`, `make status`, `make verify-portable`, and `make verify`.
